...
/**
 * A {@link org.grails.datastore.mapping.core.Session} that deals directly with persisting Mongo {@link org.bson.Document} objects
 *
 * @author Graeme Rocher
 * @since 5.0.0
 */
public class MongoDocumentSession extends MongoSession {


    public static final String MONGO_SET_OPERATOR = "$set";
    public static final String MONGO_UNSET_OPERATOR = "$unset";

    public MongoDocumentSession(MongoDatastore datastore, MappingContext mappingContext, ApplicationEventPublisher publisher) {
        super(datastore, mappingContext, publisher);
    }

    public MongoDocumentSession(MongoDatastore datastore, MappingContext mappingContext, ApplicationEventPublisher publisher, boolean stateless) {
        ...
        try {
            ...
            for (PersistentEntity persistentEntity : pendingUpdates.keySet()) {
                ...
                if(updates != null && !updates.isEmpty()) {
                    ...
                    for (PendingUpdate update : updates) {
                        ...
                        updateDoc.remove(MongoDocumentEntityPersister.MONGO_ID_FIELD);
                        updateDoc = createSetAndUnsetDoc(updateDoc);
                        final Object nativeKey = update.getNativeKey();
                        final Document id = new Document(MongoDocumentEntityPersister.MONGO_ID_FIELD, nativeKey);
                        MongoDocumentEntityPersister documentEntityPersister = (MongoDocumentEntityPersister) getPersister(persistentEntity);
            ...
            for (PersistentEntity persistentEntity : pendingDeletes.keySet()) {
                ...
                if(deletes != null && !deletes.isEmpty()) {
                    ...
                    entityWrites.add(new DeleteManyModel<Document>(new Document( MongoDocumentEntityPersister.MONGO_ID_FIELD, new Document(MongoDocumentQuery.MONGO_IN_OPERATOR, nativeKeys))));
                }
            }
    ...
    public void delete(Iterable objects) {
        ...
        for (final PersistentEntity persistentEntity : toDelete.keySet()) {
            final MongoDocumentQuery query = new MongoDocumentQuery(this, persistentEntity);
            query.in(MongoDocumentEntityPersister.MONGO_ID_FIELD, toDelete.get(persistentEntity));

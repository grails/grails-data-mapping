...
class RxGormStaticApi<D> implements RxGormAllOperations<D> {
    ...
    Observable<D> findOrCreateWhere(Map queryMap) {
        findWhere(queryMap)
            .switchIfEmpty(Observable.create({ ObservableEmitter o ->
            o.onNext(entity.javaClass.newInstance(queryMap))
            o.onComplete()
        }))
    }
    ...
    Observable<D> findOrSaveWhere(Map queryMap) {
        findWhere(queryMap)
                .switchIfEmpty(Observable.create({ ObservableEmitter emitter ->
            Thread.start {
                def instance = entity.javaClass.newInstance(queryMap)
                ((RxEntity)instance).save().subscribe(new DisposableObserver() {
                    @Override

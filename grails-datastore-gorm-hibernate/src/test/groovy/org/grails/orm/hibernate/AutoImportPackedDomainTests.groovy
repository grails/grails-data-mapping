...
class AutoImportPackedDomainTests extends AbstractGrailsHibernateTests {
    ...
    void testAutoImport() {
        ...
        def models = Model.findAll("from org.grails.orm.hibernate.AutoImportModel as m where m.name = 'Name'")
        assertTrue models.size() == 1

        models = Model.findAll("from AutoImportModel as m where m.name = 'Name'")
        assertTrue models.size() == 1

        models = Model2.findAll("from org.grails.orm.hibernate.AutoImportModel2 as m where m.name = 'Name'")
        assertTrue models.size() == 1

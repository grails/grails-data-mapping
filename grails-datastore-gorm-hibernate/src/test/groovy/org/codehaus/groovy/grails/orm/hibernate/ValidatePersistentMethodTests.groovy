...
class ValidatePersistentMethodTests extends AbstractGrailsHibernateTests {
    ...
    void testValidationAfterBindingErrors() {
        def team = new ValidateTeam()
        Errors errors = team.errors
        BeanPropertyBindingResult bindingErrors = new BeanPropertyBindingResult(team, team.class.name)
        bindingErrors.addError(new FieldError(team.class.name, "homePage", "invalidurl", true, ["typeMismatch"] as String[], [] as Object[], "invalid.url"))
        errors.addAllErrors(bindingErrors)
